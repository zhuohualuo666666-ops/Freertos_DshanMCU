...
static void RotaryEncoderTask(void *params)
{
	...
	while (1)
	{
		...
		if (rdata.speed<0){left=1;rdata.speed=0-rdata.speed;}
		else{left=0;}
//		cnt=rdata.speed/10;
//		if(!cnt)
//		{cnt=1;}
...
{		
	...
	g_xQueueRotary=xQueueCreateStatic(10,sizeof(struct rotary_data),g_ucQueueRotaryBuf,&g_xQueueRotaryStaticStructure);
