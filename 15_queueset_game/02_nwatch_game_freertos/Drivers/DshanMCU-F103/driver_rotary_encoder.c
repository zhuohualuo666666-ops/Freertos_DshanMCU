...
//    mdelay(2);
	...
	if(time-pre_time<2000000)/*当前中断事件跟上次中断时间<2ms，认为是抖动*/
	{
		pre_time=time;
	return;
	}
...
void RotaryEncoder_Init(void)
{
    ...
    g_xQueueRotary=xQueueCreateStatic(ROTARY_QUEUE_LEN,sizeof(struct rotary_data),g_ucQueueRotaryBuf,&g_xQueueRotaryStaticStructure);
}
